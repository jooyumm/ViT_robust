...
이름에 "v1"을 붙인 이유
----------------------
이 파일 하나가 01_patch_attack_detector의 전부이던 시절엔 그냥 `detector.py`였다. 이 프로젝트의 존재
...
않는다(02_patch_switch_defense가 지금 실제로 배포에 쓰는 버전이 바로 이것이므로).

원래 위치와 배경 (02_patch_switch_defense/defense/detector.py였을 때)
--------------------------------------------------------------
...
2026-09-21에 이 파일 자체가 02_patch_switch_defense/defense/에서 이 프로젝트로 옮겨졌고(git
이력 보존, 01_patch_attack_detector README "이력" 참고), 다시 여기서 `detector/topk_mass_v1.py`로
