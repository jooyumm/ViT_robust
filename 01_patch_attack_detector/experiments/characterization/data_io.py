...
def load_attention_data(npz_path):
    ...
    repr_by_group[group]['row_avg'|'col_avg'|'row_ph'|'col_ph'|'row_full_avg'] -> 대표
    이미지 분포 배열. row_full_avg만 197차원(CLS 포함, 정규화 안 됨) — 나머지는 196차원
    (patch만, 정규화됨). row_full_avg는 구버전 npz(00_extract_attention.py가 이 필드를
    추가하기 전에 생성됨)에는 없을 수 있어 있을 때만 채운다.
    """
