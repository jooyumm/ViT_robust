...
03_token_bars.py/04_column_check.py가 공유하는 그림 함수. 전부 00_extract_attention.py가
저장한 npz만 읽고 다시 attention을 계산하지 않으므로, 여기 함수들도 전부 이미 계산된
배열(분포/지표)만 받는다. GT(공격이 실제로 건드린 patch) 관련 그림은 여기 없음 —
`../../ground_truth/gt_overlay_plot.py` 참고(그쪽은 characterization 전용이 아니라
독립적으로 재사용하려고 별도 폴더로 뺐다).
"""
import numpy as np
import matplotlib
matplotlib.use('Agg')
