...
02_patch_switch_defense/system_comparison이 확정한 L=12 하나만 보는 대신, "L=12까지 봐야 신호가 확실히 분리된다"는 주장 자체를 전체 곡선으로 뒷받침/반박).
...
방법
----
1) 02_patch_switch_defense/experiments/system_comparison과 완전히 동일한 표본·공격 설정을 쓴다
...
3) L=1..12 각각에 대해 raw_at_layer(., L) + top4_mass(.)로 점수를 뽑고:
   - AUROC: eval 150개(held-out)만으로 계산 — calibration은 임계값 전용이라는 이 프로젝트의
     순환평가 방지 원칙(02_patch_switch_defense README "샘플링 감사")을 그대로 따른다.
...
`00_patch_size_tradeoff`/`02_patch_switch_defense`/`01_patch_attack_detector` 세 프로젝트가 각자 사본을 갖던 걸 2026-09-21에
...
02_patch_switch_defense/experiments/system_comparison/eval_utils.py에서 로직 변경 없이 그대로
...
# ── eval_utils.py(02_patch_switch_defense/experiments/system_comparison)에서 로직 변경 없이 복사 ──
