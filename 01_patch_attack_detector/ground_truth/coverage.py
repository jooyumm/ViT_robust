"""
ground_truth/coverage.py — 공격이 실제로 어느 patch를 건드렸는지 "눈으로 보고 추측"하지
않고, clean/adv 이미지 텐서를 직접 diff해서 계산하는 ground truth(GT) 함수. 이 폴더의
유일한 역할은 이 GT를 뽑아내는 것 — attention이나 탐지기 출력과 비교하는 로직은 여기
없다(그건 이 GT를 가져다 쓰는 별도의, 나중 단계). characterization뿐 아니라 앞으로 어떤
실험이든(탐지기 테스트 등) "공격이 진짜 어디 있었는지"가 필요하면 이 파일을 그냥 가져다
쓰면 된다.
