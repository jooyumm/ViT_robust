"""
experiments/characterization/gt_lib.py — 공격이 실제로 어느 patch를 건드렸는지 "눈으로 보고
추측"하지 않고, clean/adv 이미지 텐서를 직접 diff해서 계산하는 ground truth(GT) 함수.
...
attention_lib.py의 row_avg/col_avg/argmax_*와 같은 인덱스 규약(patch 0~195, row*ppl+col)을
써서, GT와 관측된 attention을 같은 축에서 바로 비교할 수 있게 한다.
"""
import torch
