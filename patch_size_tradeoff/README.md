...
[`patch_switch_defense`](../patch_switch_defense/)로 분리되면서 이름을 바꿨다 — 여기는 "patch size 특성 분석"에
...
## 적응형 방어 프로젝트 (`patch_switch_defense/`)

위 실험들(패치 크기 vs 강건성 특성 분석)과는 별도로, 이 발견("PatchFool에 대해 P8이 P16보다
압도적으로 강건함")을 실제 방어로 발전시키는 후속 연구는 **독립된 프로젝트**
[`../patch_switch_defense/`](../patch_switch_defense/)로 분리했다(2026-09-15) — "P16으로 추론하다가 공격이
...
[`patch_switch_defense/README.md`](../patch_switch_defense/README.md) 참고.

## 디렉토리 구조

**2026-09-21**: `src/`(models.py/dataset.py/attacks/{lavan,patch_fool}.py)는
`patch_switch_defense`/`patch_attack_detector`와 완전히 같은 내용이라 [`../src/`](../src/)(`ViT_robust`
...
pgd.py                      PGD (전역 L∞) — epsilon/alpha 계산에 쓰는 정규화 상수는
                            ...
                            이 프로젝트 전용, 공유 안 됨(PGD는 patch_switch_defense/patch_attack_detector
                            범위 밖)

../src/                    공유 라이브러리 코드 (patch_switch_defense/patch_attack_detector와 공유,
