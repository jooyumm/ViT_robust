# patch_attack_detector — ViT 패치 공격 탐지기 (공유 라이브러리)

ViT 방어 프로젝트들(`../patch_switch_defense/`의 all_switch/local_switch 등)이 공통으로 쓰는
**탐지기 자체**를 방어 로직(재분류·affine bridge)과 분리해서 독립적으로 발전시키기 위한
프로젝트다. 2026-09-21에 `patch_switch_defense/defense/detector.py`를 코드 변경 없이 그대로
옮겨와 만들었다(아래 "이력" 참고).
...
위치로 잡는다(recall@1 96.7%, `patch_switch_defense`의 §2). 점수를 calibration에서 정한 임계값과
비교해 escalate 여부를 결정한다 — 임계값 자체는 이 모듈이 만들지 않고, 매 실험이 자기
calibration 표본으로 정한다. 자세한 배경·한계(예: LaVAN 비탐지)는
[`../patch_switch_defense/README.md`](../patch_switch_defense/README.md)의 "공유 인프라: 탐지 + 위치특정"
절 참고.
...
자체는 바뀌지 않는다(`patch_switch_defense`가 실제 배포에 쓰는 버전이 바로 이것이므로).

## 핵심 결과 요약

**레이어별 AUROC/FPR/recall sweep** (`experiments/layer_sweep/`, PatchFool, n=250/seed=42,
`patch_switch_defense`의 system_comparison과 동일 표본·공격): AUROC-vs-layer 곡선은 **단조증가가
...
```
ViT_robust/
  src/                         모델/데이터셋/공격 코드 — patch_size_tradeoff/patch_switch_defense/
                                patch_attack_detector 세 프로젝트가 공유(아래 "공유 src/" 참고)

patch_attack_detector/
  detector/                    탐지기 구현들 — 방어 로직(patch_switch_defense)과 분리해서 이
                                프로젝트에서만 독립적으로 발전시킨다
    topk_mass_v1.py              현재 patch_switch_defense가 실배포에 쓰는 v1(raw attention
...
`detector/topk_mass_v1.py`는 `patch_switch_defense/defense/detector.py`에서 코드 한 글자도
...
`patch_switch_defense`의 `src/`(models.py, dataset.py, attacks/)를 cross-import했었다.

**2026-09-21**: `patch_size_tradeoff`/`patch_switch_defense`/`patch_attack_detector` 세 프로젝트가 완전히 같은
...
공유로 바뀐 지금도 `detector/`는 여전히 이 프로젝트 소유이고, `patch_switch_defense`가
cross-import하는 유일한 대상이다 — 아래 절 참고.

## 쓰는 곳 (cross-project import, `detector/`만 해당)

`patch_switch_defense`의 실험 스크립트는 `sys.path`에 `patch_attack_detector` ROOT를 추가해
`from detector.topk_mass_v1 import ...`로 직접 import한다. 현재 이 방식을 쓰는 곳은
`patch_switch_defense/experiments/{system_comparison,cost_comparison,
...
패키지 이름 충돌 방지를 위한 sys.path 순서" 문제는 더 이상 없다 — `patch_switch_defense`와
`patch_attack_detector` 둘 다 같은 `ViT_robust/src/`를 가리키므로 순서 무관하게 항상 같은 곳으로
resolve된다.

## 레이어별 Top-K Mass sweep (`experiments/layer_sweep/`)

지금까지 `patch_switch_defense`에서 확인된 지점은 L=6(AUROC 0.639)과 L=12(0.879, §1, n=30
...
`patch_switch_defense`의 system_comparison 발표치(threshold=0.5116, FPR=13.3%, recall=72.7%)와
...
1. **LaVAN은 attention 구조를 거의 안 건드린다.** 모든 레이어에서 LaVAN의 중앙값이 clean과
   ...
   layer_comparison_top1_row.png`](results/characterization/01_layer_comparison_top1_row.png)
   에서 clean/LaVAN 선이 12개 레이어 내내 거의 포개진다. `patch_switch_defense` README가 이미
...
2. **PatchFool의 신호는 L=11~12에서만 뚜렷하고, 그마저도 이미지마다 다르다.** L=10까지는
   ...
   characterization/hist_top1_row_by_layer.png`](results/characterization/01_hist_top1_row_by_layer.png)
