...
파일이 유일하게 GPU가 필요한/느린 단계다 — 01_concentration.py와 02_sink_position.py는
둘 다 이 npz만 읽어서 각자 다른 질문에 답한다(공격을 다시 생성하지 않음).
...
from attention_lib import (
    collect_full_layer_attn, row_distribution, col_distribution,
...
def main():
    ...
    for name in GROUPS:
        ...
        metrics, row_avg, col_avg, row_ph, col_ph = process_group(
